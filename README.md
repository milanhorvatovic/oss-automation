...
This repository is the first consumer of its own Dependabot flavors, and both of its callers reference the workflows locally rather than at a SHA. That is safe rather than expedient: the policy caller runs on `pull_request_target`, so its delegation resolves against the base branch and a pull request cannot supply the code the App credentials reach — which is why the policy is checkout-free and must stay so. Pinning at the first release remains open as dogfooding rather than as a fix (see `.github/workflows/dependabot-automerge.yaml`).
