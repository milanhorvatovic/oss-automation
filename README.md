...
This repository is the first consumer of its own Dependabot flavors. Until the first tagged release gives a pin comment a version to name, its two callers reference the workflows locally rather than at a SHA. The scheduled reconcile caller pays nothing for that; the `pull_request` policy caller declines to delegate at all while a pull request edits `.github/**`, because a local reference would otherwise let such a pull request supply the policy code and the action pins that judge it. That refusal outlives the local reference: a caller is read from the merge ref whether its `uses:` names a path or a pinned SHA, so a pull request bumping the pin still selects the policy that judges the bump (see `.github/workflows/dependabot-automerge.yaml`).
